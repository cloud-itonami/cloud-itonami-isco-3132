(ns treatment-ops.actor
  "TreatmentOpsActor — the ISCO-08 3132 incinerator and water treatment
  plant operations coordination actor as a `langgraph.graph/state-graph`
  ...
  COORDINATION WORKFLOW only. It does NOT directly control incinerators,
  chemical injection, thermal treatment, or emergency shutdown — those
  remain exclusively under licensed plant operators' human authority.
  The Governor enforces this hard boundary (see `treatment-ops.governor`).
  ...
  The unconditional invariant: the TreatmentOpsAdvisor can never directly
  commit a record or dispatch a robot action the TreatmentOpsGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [treatment-ops.advisor :as advisor]
            [treatment-ops.governor :as governor]
            [treatment-ops.store :as store]))

(defn build-graph
  "Build a compiled TreatmentOpsActor graph. `store` implements
  `treatment-ops.store/Store`. `advisor` implements `treatment-ops.advisor/Advisor`
